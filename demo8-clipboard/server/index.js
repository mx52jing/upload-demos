...
app.use(async ctx => {
    ...
    */
    try {
        ...
        /*
        * 多个文件上传，
        * demo8ClipBoard就是一个数组，而不是一个对象
        * */
        let { demo8ClipBoard = [] } = files,
            result = []
        /* 这里要注意一下 如果只上传了一个文件, demo8ClipBoard就不是一个数组，要兼容一下 */
        if (!Array.isArray(demo8ClipBoard)) {
            demo8ClipBoard = [demo8ClipBoard]
        }
        !!demo8ClipBoard && demo8ClipBoard.forEach(item => {
            /*
             * path 路径
             * name 文件名称 例如 a.png
             * size 文件大小
